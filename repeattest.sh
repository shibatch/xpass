...
while true; do
	...
	clang-10 -DTEST -ffast-math -O1 $BUILDDIR/mp_generated_"$$"_$type.c -emit-llvm -S -o $BUILDDIR/mp_generated_"$$"_$type.ll
	valgrind -q opt-10 -O1 -load=$BUILDDIR/lib/libMathPeephole.so -ffast-math -lint $BUILDDIR/mp_generated_"$$"_$type.ll -o $BUILDDIR/mp_generated_"$$"_$type.bc 2>>$BUILDDIR/valgrind.out
